"""Perform point navigation task in habitat(-lab) using multi processing
...
def make_image(observations, info):
    ...
    equirect_depth_im = display_obs(
        np.squeeze(observations["equirect_depth"], axis=2), "depth"
    )
    equirect_semantic_im = display_obs(
        np.squeeze(observations["equirect_semantic"], axis=2), "semantic"
    )
    top_down_rgb = observations["top_down_rgb"]
    top_down_semantic = display_obs(
        np.squeeze(observations["top_down_semantic"], axis=2), "semantic"
    )
    ...
    pil_img = (
        Image.open(buf)
        .convert("RGB")
        .resize((rgb_im.shape[1] * 4, rgb_im.shape[0] * 4))
    )
    buf.close()
...
def slice_env_episodes(env, episodes_index):
    # overriding to skip a few episodes
    env.episode_iterator = itertools.islice(
        env._episode_iterator, episodes_index[0], episode_index[1]
    )
...
def run_pointnav_episodes(
    ...
    episodes_index: Tuple[int, int],
    rank: int,
):
    dataset_collector = DatasetCollector(
        offline_dataset_config, online_env_config, episode_start_index=episode_index[0]
    )
    ...
    with SimpleEnv(config=env_config) as env:
        ...
        slice_env_episodes(env, episode_index)
        ...
        pbar = (
            tqdm(range(episodes_index[0], episodes_index[1]), desc="Episodes")
            if rank == 0
            else range(episodes_index[0], episodes_index[1])
        )
        for episode in pbar:
            ...
            while not env.episode_over:
                ...
                output_image = (
                    make_image(observations, info)
                    if offline_dataset_config["record_video"]
                    else None
                )
        ...
        # )
        dataset_collector.to_json(rank)
...
if __name__ == "__main__":
    ...
    if config["num_workers"] == 1:
        run_pointnav_episodes(
            config["offline_dataset"],
            config["online_env"],
            config["agent_config"],
            (0, config["online_env"]["num_episodes"]),
        )
    else:
        episode_index_for_process = equal_slice(
            config["online_env"]["num_episodes"], config["num_workers"]
        )
        processes = []
        for i in range(config["num_workers"]):
            episode_index = episode_index_for_process[i]
            p = Process(
                target=run_pointnav_episodes,
                args=(
                    config["offline_dataset"],
                    config["online_env"],
                    config["agent_config"],
        ...
        # combind json files from each process
        DatasetCollector.merge_jsons(
            config["offline_dataset"]["dataset_dir"], config["num_workers"]
        )

    # pack dataset into seqrecord
    DatasetCollector.to_seqrecord(
        recorddir=config["offline_dataset"]["seqrecord_dir"],
        features=config["offline_dataset"]["seqrecord_features"],
        dataset_pretransform_module=config["offline_dataset"][
            "dataset_pretransform_module"
        ],
        dataset_dir=config["offline_dataset"]["dataset_dir"],
    )
